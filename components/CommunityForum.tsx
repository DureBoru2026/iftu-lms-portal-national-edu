...
const CommunityForum: React.FC<{ currentUser: User }> = ({ currentUser }) => {
  ...
  useEffect(() => {
    const unsubDiscussions = dbService.subscribeToDiscussions(setDiscussions);
    const unsubUsers = dbService.subscribeToUsers((userList) => {
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="flex items-center gap-8 pt-4 border-t-2 border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-white rounded-xl border-4 border-black overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                      <img 
                        src={users[d.authorId]?.photo || d.authorPhoto || `https://api.dicebear.com/7.x/avataaars/svg?seed=${d.authorId}`} 
                    ...
                    <div>
                      ...
                      <div className="text-sm font-black uppercase tracking-tight">{users[d.authorId]?.name || d.authorName}</div>
                    </div>
                  </div>
