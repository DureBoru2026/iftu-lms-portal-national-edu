...
const ExamEngine: React.FC<ExamEngineProps> = ({ exam, onComplete, onCancel }) => {
  ...
  useEffect(() => {
    const checkAvailability = () => {
      ...
      if (exam.startTime && now < new Date(exam.startTime)) {
        setCanStart(false);
        setWaitingReason(`Exam starts at ${new Date(exam.startTime).toLocaleString()}`);
        return;
      }
      if (exam.endTime && now > new Date(exam.endTime)) {
        setCanStart(false);
        setWaitingReason('Exam has already ended.');
        return;
      }
      if (exam.hasWaitingRoom) {
        setCanStart(false);
        setWaitingReason('Waiting for administrator to begin the session...');
        return;
      }
      setCanStart(true);
      setWaitingReason("");
  ...
  }, [exam.startTime, exam.endTime, exam.hasWaitingRoom]);

  useEffect(() => {
    // SEB Verification Logic
    // In a real scenario, this would check X-SafeExamBrowser-RequestHash
    // For this build, we verify the user agent
    ...
    if (exam.sebRequired) {
      if (isSEB) {
        ...
        setAlerts(prev => [...prev, `[${new Date().toLocaleTimeString()}] CRITICAL: SEB LOCKDOWN MISMATCH.`]);
      }
    } else {
      setIsSebVerified(true); // Not required
    }
  }, [exam.sebRequired]);
      ...
      <div className="fixed inset-0 z-[10000] bg-slate-100 flex items-center justify-center p-6 text-center overflow-auto">
        ...
        >
            ...
            <div className="max-w-xl mx-auto pt-6 space-y-6">
              <div className="flex flex-col gap-4">
                 ...
                 <div className="flex items-center justify-center gap-3">
                    {exam.sebRequired && (
                      <div className={`px-4 py-1 border-2 border-black rounded-lg text-[10px] font-black uppercase flex items-center gap-2 ${isSebVerified ? 'bg-green-600 text-white' : 'bg-rose-600 text-white animate-pulse'}`}>
                        <ShieldAlert size={10} />
                        {isSebVerified ? 'SEB: AUTHENTICATED' : 'SEB: REQUIRED'}
                      </div>
                    )}
                   ...
                   <div className="px-4 py-1 bg-purple-100 border-2 border-black rounded-lg text-[10px] font-black uppercase text-purple-700">
                     S{exam.semester} / {exam.academicYear}
                   </div>
                 </div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-8 pt-10">
            ...
            <motion.button 
              whileHover={(isSebVerified && canStart) ? { scale: 1.05 } : {}}
              whileTap={(isSebVerified && canStart) ? { scale: 0.95 } : {}}
              disabled={!isSebVerified || !canStart}
              onClick={enterSecureMode} 
              className={`py-10 border-8 border-black rounded-[3.5rem] font-black uppercase text-3xl shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] transition-all flex flex-col items-center justify-center gap-2 ${(isSebVerified && canStart) ? 'bg-blue-600 text-white hover:bg-blue-700 active:shadow-none active:translate-y-2' : 'bg-gray-300 text-gray-500 cursor-not-allowed opacity-50'}`}
            >
              {isSebVerified ? (
                canStart ? (
                  ...
                  <div className="text-xl">WAITING ROOM</div>
                )
              ) : (
                <div className="text-xl">SEB LINK NEEDED</div>
              )}
            </motion.button>
          </div>
          {!isSebVerified && (
            <div className="bg-rose-100 border-4 border-rose-600 p-6 rounded-3xl animate-fadeIn">
               <p className="text-rose-600 font-black uppercase italic text-sm">
                 Security Breach: This exam requires the Safe Exam Browser (SEB) for entry. 
                 Launch the portal via the SEB application to proceed.
               </p>
            </div>
          )}
          {isSebVerified && !canStart && (
            <div className="bg-orange-100 border-4 border-orange-600 p-6 rounded-3xl animate-fadeIn">
               <p className="text-orange-600 font-black uppercase italic text-sm animate-pulse">
                 {waitingReason}
               </p>
            </div>
          )}
