  ...
  const filteredAssignments = useMemo(() => {
    return assignments.filter(a => {
      ...
      if (currentUser.role === 'student') {
        ...
        if (activeTab === 'pending') return !sub || sub.status === 'pending' || sub.status === 'resubmission_required';
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4">
                    ...
                    {currentUser.role === 'student' ? (
                      <div className={`p-4 rounded-2xl border-4 border-black ${
                        ...
                        <div className="text-[8px] font-black uppercase text-gray-400 mb-1 font-sans">Assignment Status</div>
                        <select 
                          className="w-full bg-transparent text-[10px] font-black uppercase italic outline-none cursor-pointer"
                          value={submission?.progressStatus || 'Not Started'}
                          onChange={async (e) => {
                            const newStatus = e.target.value as any;
                            const sub: AssignmentSubmission = submission || {
                              id: `${assignment.id}_${currentUser.id}`,
                              assignmentId: assignment.id,
                              studentId: currentUser.id,
                              studentName: currentUser.name,
                            ...
                            await dbService.syncSubmission({ ...sub, progressStatus: newStatus });
                          }}
                        >
                          {['Not Started', 'In Progress', 'Completed', 'Ready for Submission'].map(s => (
            ...
            <div className="p-10 md:p-14 space-y-10">
              ...
              <div className="bg-gray-50 border-4 border-black p-8 rounded-3xl space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black uppercase text-gray-400">Personal Progress:</span>
                    <select 
                      className="bg-white border-2 border-black rounded-lg px-2 py-1 text-[10px] font-black uppercase italic outline-none"
                      value={getSubmissionForAssignment(selectedAssignment.id)?.progressStatus || 'Not Started'}
                      onChange={async (e) => {
                        const newStatus = e.target.value as any;
                        const submission = getSubmissionForAssignment(selectedAssignment.id);
                        const sub: AssignmentSubmission = submission || {
                          id: `${selectedAssignment.id}_${currentUser.id}`,
