...
const CourseViewer: React.FC<CourseViewerProps> = ({ 
  ...
  const getLessonIcon = (lesson: Lesson) => {
    switch (lesson.contentType) {
      ...
      case 'tutor': return <Brain className="w-5 h-5" />;
          ...
          <div className="max-w-5xl mx-auto space-y-12 pb-24">
            ...
            {activeLesson.id.includes('oral') ? (
              ...
              <>
                ...
                ) : activeLesson.contentType === 'tutor' ? (
                  <div className="w-full bg-indigo-50 border-8 border-black rounded-[3.5rem] p-12 md:p-20 shadow-[25px_25px_0px_0px_rgba(79,70,229,1)] space-y-12 animate-fadeIn">
                    <div className="text-center space-y-6">
                       <div className="w-32 h-32 bg-indigo-600 text-white rounded-[2.5rem] border-4 border-black flex items-center justify-center text-6xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] mx-auto animate-pulse">🤖</div>
                       <h3 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter">AI Tutor Interactive Module</h3>
                       <p className="text-xl font-bold text-gray-500 uppercase tracking-widest max-w-2xl mx-auto">
                         This specialized module is supervised by IFTU's National AI. You will engage in a deep-dive dialogue about the topic below.
                       </p>
                    </div>
                    <div className="bg-white border-4 border-black p-8 rounded-3xl space-y-6">
                       <h4 className="text-2xl font-black uppercase italic text-indigo-600">Module Context: {activeLesson.title}</h4>
                       <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed font-medium line-clamp-4">
                         <Markdown>{activeLesson.content}</Markdown>
                       </div>
                    </div>
                    <button 
                      onClick={() => onOpenTutor?.(activeLesson.content, activeLesson.title, `You are acting as a specialized tutor for the lesson: ${activeLesson.title}. The student is currently studying: ${activeLesson.content}. Please help them understand these concepts deeply, provide examples, and walk them through any kinematic calculations they might find difficult. Stay focused on Linear Motion unless they ask for broader applications.`)}
                      className="w-full py-10 bg-indigo-600 text-white rounded-[3rem] border-8 border-black font-black uppercase text-3xl shadow-[15px_15px_0px_0px_rgba(34,197,94,1)] hover:translate-y-2 transition-all flex items-center justify-center gap-6"
                    >
                      <Brain size={48} />
                      Initiate AI Session →
                    </button>
                    <p className="text-center text-[10px] font-black uppercase text-gray-400 tracking-[0.3em]">Authorized by IFTU National AI Lab</p>
                  </div>
